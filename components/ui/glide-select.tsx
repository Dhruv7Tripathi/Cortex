...
const GlideSelect = ({
  ...
  return (
    <div
      className={cn(
        "relative",
        "flex w-full max-w-107.5 flex-col justify-center",
        "rounded-md px-0.5 pt-0.5 pb-0.5",
      )}
    >
      <div
        className={cn(
          "flex w-full flex-col justify-center",
          "border border-neutral-200 dark:border-neutral-900/50",
          "rounded-md px-1.5 pt-1.5 pb-1.5",
        )}
      >
        <div
          className={cn(
            "flex items-center justify-between",
            "rounded-t-[6px] py-2.5 pr-2.5 pl-4.5",
            "border-x border-t border-neutral-200 dark:border-neutral-800/60",
            "bg-neutral-100 dark:bg-[#101010]",
            "text-[13px] text-neutral-700 dark:text-neutral-300",
          )}
        >
          <div className="flex items-center gap-2">
            <VscListSelection className="mt-0.5 size-3.5 text-neutral-600 dark:text-neutral-400" />
          ...
          <span>
            <HiOutlineSelector className="mt-0.5 size-3.5 text-neutral-600 dark:text-neutral-400" />
          </span>
        </div>
        <div
          className={cn(
            "relative",
            "flex min-h-58.75 w-full flex-col gap-1 overflow-hidden",
            "rounded-b-[5px] border border-neutral-200 px-3 py-2 dark:border-neutral-800/60",
            "bg-neutral-100 dark:bg-[#101010]",
          )}
        >
          <div
            ...
            className="absolute inset-x-0 left-0 mx-auto h-14 w-[94%] rounded-md bg-neutral-200 dark:bg-neutral-900"
          />
          ...
          {steps.map((step, idx) => (
            <div
              key={idx}
              className={cn(
                "text-primary absolute flex h-14 w-full justify-center gap-2 rounded-md px-1 py-2",
              )}
              style={{ top: `${positions[idx]}px` }}
            >
              <div className="mt-1 flex h-full w-[5%] justify-center">
              ...
              <div className="flex h-full w-full flex-col gap-1 text-xs">
                <p className="text-primary text-xs">{step.title}</p>
                <p className="text-[11px] text-neutral-600 dark:text-neutral-400">
                  {step.description}
                </p>
              </div>
